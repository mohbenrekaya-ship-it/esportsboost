# -*- coding: utf-8 -*-
"""Vercel serverless function: GET /api/auth/:provider.

Begins social sign-in: redirects the browser to the provider's consent screen
and drops the signed CSRF `state` cookie. The provider is read from the path, so
the same file serves /api/auth/google and /api/auth/discord. Thin shell around
oauth.dispatch — logic shared verbatim with the local server (site/serve.py).
"""
import json
import os
import sys
