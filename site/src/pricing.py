...
# ── The service fee ────────────────────────────────────────────────────────
# A flat amount added to every order as its OWN Stripe line item, on every
# product — boosts, coaching and accounts alike.
#
# ⚠ IT IS DELIBERATELY NOT IN quote(), AND THAT IS THE WHOLE POINT OF IT.
# The site's checkout summary, the sticky mobile bar and the closing band all
# quote quote()'s total, so this figure is first seen on Stripe's own page,
# after the buyer has read and accepted a total that does not include it.
#
# That is the business's explicit call (2026-09-15), taken with both of the
# following on the table. Recorded here rather than argued again:
#
#   • IT CONTRADICTS SHIPPED COPY, in three languages. `gp_steps()` puts
#     "The number you see is the number you pay. Nothing is added later" on all
#     nine game pages under a chip reading "Price fixed at checkout", and
#     GUARANTEE's `price` answer says "the number on the button is the number
#     charged, and nothing is added later". Both have `fr` and `de` values and
#     both render on /guarantee.html and /legal/terms.html. Those sentences are
#     FALSE for as long as this constant is non-zero. Either the fee moves into
#     quote() or that copy has to change — they cannot both stand.
#   • IT IS DRIP PRICING. The UK's DMCC Act 2024 prohibits a mandatory fee
#     disclosed after the headline price, and the EU Consumer Rights Directive
#     requires the total inclusive of mandatory fees BEFORE the order is placed.
#     A large share of this site's traffic is EU and UK.
#
# ⚠ THE OFF SWITCH IS ZERO, and it needs no deploy: `ESB_SERVICE_FEE=0` adds no
# line item and changes nothing else about the session. Set it in the Vercel
# environment to kill the fee in production immediately.
#
# ⚠ The SAME DIGITS in every currency, never converted — the `fixed` rule the
# accounts price table already follows. A converted fee would read $2.99 /
...
def service_fee_for(currency):
    """`(currency_code, integer minor units)` for the fee's own line item, or a
    zero amount when it is switched off.

    Flat and unconverted — `fixed=True` — for the reason on SERVICE_FEE, and to
    the cent because the figure has them. A caller adds the line item only when
    the amount is positive."""
    if SERVICE_FEE <= 0:
        cur = str(currency or "usd").strip().lower()
        return (cur if cur in CHARGE_RATES else "usd"), 0
    return charge_for(SERVICE_FEE, currency, cents=True, fixed=True)
