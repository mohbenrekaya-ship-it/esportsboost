...
(function () {
  ...
  var ESB_I18N = window.ESB_I18N = {
    fr: {
      ...
      "SPLIT15 takes 15% off the whole catalogue with nothing to type. Each game page also carries bundle climbs at 19% to 38% off, and a bundle replaces the code rather than adding to it — there is only ever one discount on an order, and it is the larger of the two.":
        "SPLIT15 enlève 15 % sur tout le catalogue, sans rien à saisir. Chaque page de jeu propose aussi des packs de montée à −19 % à −38 %, et un pack remplace le code au lieu de s’y ajouter — il n’y a jamais qu’une seule remise par commande, et c’est la plus avantageuse des deux.",
    ...
    de: {
      ...
      "A division is not the same amount of work in every game. Ladders are different lengths, matches are different lengths, and one rung near the top of a ladder can cost several near the bottom of another. Each title carries its own multiplier, and it is on screen before you sign in: the cheapest single division is $4 on Valorant and $10 on Counter-Strike 2.":
        "Eine Division bedeutet nicht in jedem Spiel gleich viel Arbeit. Ladders sind unterschiedlich lang, Matches ebenso, und eine Sprosse nahe der Spitze einer Ladder kann so viel kosten wie mehrere am unteren Ende einer anderen. Jeder Titel hat seinen eigenen Multiplikator, und er steht vor der Anmeldung auf dem Bildschirm: die günstigste einzelne Division kostet 4 $ bei Valorant und 10 $ bei Counter-Strike 2.",
      ...
      "SPLIT15 takes 15% off the whole catalogue with nothing to type. Each game page also carries bundle climbs at 19% to 38% off, and a bundle replaces the code rather than adding to it — there is only ever one discount on an order, and it is the larger of the two.":
        "SPLIT15 zieht 15 % vom gesamten Katalog ab, ohne dass du etwas eingeben musst. Jede Spielseite führt außerdem Bundle-Aufstiege mit 19 % bis 38 % Rabatt, und ein Bundle ersetzt den Code, statt sich dazuzuaddieren — es gibt immer nur einen Rabatt pro Bestellung, und zwar den größeren von beiden.",
